...
class _QuizEditorPageState extends State<QuizEditorPage> {
  ...
  void initState() {
    super.initState();
    final first = _createQuestion(QuestionType.multipleChoice);
    questions = [first];
    activeId = first.id;
  ...
  int get _totalPoints =>
      questions.fold(0, (sum, q) => sum + (q.points < 0 ? 0 : q.points));
  ...
  Widget _buildMultipleChoiceEditor(QuestionModel q) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text(
